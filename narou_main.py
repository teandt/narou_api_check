...
def parse_limit_count(s: str) -> int:
    """LIMIT_COUNT を1以上の整数としてチェックする"""
    limit_count = int(s)
    if limit_count < 1:
        raise ValueError
    return limit_count
...
def build_parser() -> ap.ArgumentParser:
    ...
    parser.add_argument(
        '-lm',
        nargs=3,
        help="指定された開始年～終了年のタイトル長の平均を出力します。LIMIT_COUNTでTOP N件を指定します。 -lm [start year] [end year] [limit count]",
        metavar=('START_YEAR', 'END_YEAR', 'LIMIT_COUNT')
    )
    parser.add_argument(
        '-lh',
        nargs=2,
        help="指定された年のタイトル長のヒストグラムを出力します。LIMIT_COUNTでTOP N件を指定します。-lh [year] [limit count]",
        metavar=('YEAR', 'LIMIT_COUNT')
    )
    parser.add_argument(
        '-nt',
        nargs=2,
        help="指定された開始年～終了年の連載／短編の数を出力します。-nt [start year] [end year]",
        type=year4_type,
...
def main(argv=None):
    ...
    if args.lm:
        try:
            ...
            limit_count = parse_limit_count(args.lm[2])
        except ValueError:
    ...
    if args.lh:
        try:
            ...
            limit_count = parse_limit_count(args.lh[1])
        except ValueError:
